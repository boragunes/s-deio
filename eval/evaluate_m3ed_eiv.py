...
def read_imu(path, imu_name):
    with h5py.File(path, "r") as h5:
        ...
        gyro = h5[f"{imu_name}/omega"][()]
        ts = h5[f"{imu_name}/ts"][()] / 1e6

    all_imu = np.hstack((ts[:, None], gyro, accel))
    all_imu[:, 1:4] *= 180 / np.pi  # Convert gyro to degrees
...
def main():
    ...
    parser.add_argument("--imu", default="/ovc/imu")
    ...
    with torch.no_grad():
        ...
        slam.state.set_imu_params([0.16*100, 0.05*100, 0.003*10, 4.0e-5*10])
